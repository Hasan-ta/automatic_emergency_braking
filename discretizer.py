...
@dataclass
class GapEgoAccelDiscretizer:
    """
    Discretizer for observation: [gap, v_ego, a_ego, v_npc]

    Each dimension has its own 1D bin edges. We map continuous values to
    bin indices via np.digitize, then pack (i_gap, i_ve, i_ae, i_vn)
    into a single flat state index s ∈ {0, ..., S-1}.
    """

    gap_bins: np.ndarray      # shape (Ng+1,)
    v_ego_bins: np.ndarray    # shape (Nv_e+1,)
    a_ego_bins: np.ndarray    # shape (Na_e+1,)
    v_npc_bins: np.ndarray    # shape (Nv_n+1,)
    ...
    def shape(self) -> Tuple[int, int, int, int]:
        """
        Number of discrete bins in each dimension:
        (Ng, Nv_ego, Na_ego, Nv_npc)
        """
        return (
            len(self.gap_bins) - 1,
            len(self.v_ego_bins) - 1,
            len(self.a_ego_bins) - 1,
            len(self.v_npc_bins) - 1,
        )

    def num_states(self) -> int:
        Ng, Nv_e, Na_e, Nv_n = self.shape()
        return Ng * Nv_e * Na_e * Nv_n
    ...
    def obs_to_indices(self, obs: np.ndarray) -> Tuple[int, int, int, int]:
        """
        obs: array-like (4,) = [gap, v_ego, a_ego, v_npc]
        Returns the 4D bin indices (i_gap, i_ve, i_ae, i_vn).
        """
        gap, v_ego, a_ego, v_npc = [float(o) for o in obs]
        i_g  = self._bin_index(gap,   self.gap_bins)
        i_ve = self._bin_index(v_ego, self.v_ego_bins)
        i_ae = self._bin_index(a_ego, self.a_ego_bins)
        i_vn = self._bin_index(v_npc, self.v_npc_bins)
        return i_g, i_ve, i_ae, i_vn

    def values_to_indices(
        self, gap: float, v_ego: float, a_ego: float, v_npc: float
    ) -> Tuple[int, int, int, int]:
        return self.obs_to_indices([gap, v_ego, a_ego, v_npc])

    # ---------- pack/unpack flat state index ----------

    def indices_to_state(self, i_g: int, i_ve: int, i_ae: int, i_vn: int) -> int:
        """
        Pack 4D indices -> flat state id.

        Flattening order matches:
          s = (((i_g * Nv_e + i_ve) * Na_e) + i_ae) * Nv_n + i_vn
        """
        Ng, Nv_e, Na_e, Nv_n = self.shape()
        s = (((i_g * Nv_e + i_ve) * Na_e) + i_ae) * Nv_n + i_vn
        return int(s)
    ...
    def state_to_indices(self, s: int) -> Tuple[int, int, int, int]:
        """
        Unpack flat state id -> 4D indices (i_gap, i_ve, i_ae, i_vn).
        """
        Ng, Nv_e, Na_e, Nv_n = self.shape()
        s = int(s)

        i_g  = s // (Nv_e * Na_e * Nv_n)
        r1   = s %  (Nv_e * Na_e * Nv_n)
    ...
    def obs_to_state(self, obs: np.ndarray) -> int:
        ...
        Continuous observation -> flat discrete state id.
        """
        i_g, i_ve, i_ae, i_vn = self.obs_to_indices(obs)
        return self.indices_to_state(i_g, i_ve, i_ae, i_vn)

    def values_to_state(
        self, gap: float, v_ego: float, a_ego: float, v_npc: float
    ) -> int:
        i_g, i_ve, i_ae, i_vn = self.values_to_indices(gap, v_ego, a_ego, v_npc)
        return self.indices_to_state(i_g, i_ve, i_ae, i_vn)

    # ---------- centers & iteration helpers ----------
    ...
    def state_centers(self) -> np.ndarray:
        """
        Returns array of shape (S, 4) with continuous centers:
          [gap, v_ego, a_ego, v_npc] for each discrete state.
        """
        gap_c  = self._bin_centers(self.gap_bins)
        ve_c   = self._bin_centers(self.v_ego_bins)
        ae_c   = self._bin_centers(self.a_ego_bins)
        vn_c   = self._bin_centers(self.v_npc_bins)

        G, VE, AE, VN = np.meshgrid(
            gap_c, ve_c, ae_c, vn_c, indexing="ij"
        )
        centers = np.stack([G, VE, AE, VN], axis=-1)  # (Ng, Nv_e, Na_e, Nv_n, 4)
        return centers.reshape(-1, 4)                 # (S, 4)

    def index_tuples(self) -> List[Tuple[int, int, int, int]]:
        """
        Returns list of all (i_gap, i_ve, i_ae, i_vn) index tuples.
        Order matches the flat indexing.
        """
        Ng, Nv_e, Na_e, Nv_n = self.shape()
        ...
        for i_g in range(Ng):
            for i_ve in range(Nv_e):
                for i_ae in range(Na_e):
                    for i_vn in range(Nv_n):
                        tuples.append((i_g, i_ve, i_ae, i_vn))
        return tuples
    ...
    def from_ranges(
        ...
        n_a_ego: int = 10,
        n_v_npc: int = 20,
    ) -> "GapEgoAccelDiscretizer":
        """
        Build a uniform-grid discretizer for [gap, v_ego, a_ego, v_npc].
        """
        gap_bins  = np.linspace(gap_min, gap_max, n_gap + 1,   dtype=np.float64)
        v_ego_bins = np.linspace(v_min,  v_max,  n_v_ego + 1, dtype=np.float64)
        a_ego_bins = np.linspace(a_min,  a_max,  n_a_ego + 1, dtype=np.float64)
        v_npc_bins = np.linspace(v_min,  v_max,  n_v_npc + 1, dtype=np.float64)
        return cls(gap_bins, v_ego_bins, a_ego_bins, v_npc_bins)
